...
export function getAccentColors(platformId) {
    return [PLATFORMS[platformId].defaultColor, ...[0, 30, 60, 150, 180, 210, 240, 270, 330].map(hslToHex)];
}
    ...
    try {
        ...
        if (platform.id === "spigot") {
            ...
            if (pathParts[0] === "resources" && pathParts[1] === "authors" && pathParts[2]) {
                ...
                if (/^\d+$/.test(pathParts[2])) return { platform: platform.id, type: "author", id: pathParts[2] };
                ...
                if (/^\d+$/.test(pathParts[1])) return { platform: platform.id, type: "resource", id: pathParts[1] };
            }
            return null;
        }
